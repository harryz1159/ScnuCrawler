...
public class TencentCrawler {
   ...
   // private  int b = 0;				//评论条数
	 ...
	 * @return 
	 */
	public static ArrayList<MicroblogData> getUserStatusesByName(String name) throws Exception
	{//
		int StatusPageCounts = 70;               //翻页微博数,最多每页70条
	    UserAPI userAPI = new UserAPI(oAuth.getOauthVersion());
		StatusesAPI statusesAPI = new StatusesAPI(oAuth.getOauthVersion());
		String Userjson = userAPI.otherInfo(oAuth, "json", name, "");      //这一句调用了user接口
		JSONObject Userobj = new JSONObject(Userjson);
	     ...
	     * 以下代码段没有调用API
	     */
	    System.out.println("大约有" + Userobj.getJSONObject("data").getInt("tweetnum") + "条微博。");
	    Thread.sleep(5 * 1000);
	    ...
	    System.out.println("获取了" + a +"条微博。");		//最后一页总有若干条微博数据不会返回。即使某账户只有3条微博也只返回2条。应该是腾讯服务器那边的原因。
	    statusesAPI.shutdownConnection();			//关闭连接
